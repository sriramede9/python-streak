...
df = pd.DataFrame({
    ...
    'merchant_category': np.random.choice(['grocery', 'electronics', 'travel', 'crypto_exchange', 'luxury'], size=n_tx),
    'ip_country': np.random.choice(['US', 'CA', 'UK', 'DE', 'RU', 'CN', 'NG', np.nan], p=[0.6, 0.15, 0.1, 0.05, 0.04, 0.03, 0.02, 0.01], size=n_tx),
...
df['tx_count_10m'] = (
    df.groupby('user_id')
      .rolling('10min', on='timestamp')['timestamp']
      .count()
      .values
)
...
df.head()
df['merchant_risk_score']=df.groupby('merchant_category')['is_fraud'].transform(lambda x: x.mean())
...
condition_fraud = (df['merchant_category'].isin(['crypto_exchange', 'luxury']) & ~(df['ip_country'].isin(['US', 'CA'])))
