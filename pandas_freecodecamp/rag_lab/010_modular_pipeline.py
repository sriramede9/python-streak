...
class RecursiveChunker:
    ...
    def split_text(
        ...
        for i, c in enumerate(raw_chunks):
            chunk_content = (
                f"...{raw_chunks[i - 1][-self.overlap :]} {c}"
...
class HybridRetriever:
    ...
    def index_chunks(self, chunks: List[DocumentChunk]):
        ...
        print(
            f"📦 Successfully indexed {len(chunks)} chunks across Sparse & Dense indices."
        )
    ...
    def search_hybrid(
        ...
        dense_res = self.collection.query(
            query_texts=[query], n_results=min(top_k, len(self.chunks_map))
        )
        ...
        return [
            ScoredChunk(
                chunk=self.chunks_map[cid], score=score, retrieval_stage="hybrid_rrf"
            )
...
class ProductionRAGPipeline:
    ...
    def run(self, query: str, candidate_k: int = 8, final_n: int = 2) -> Dict[str, Any]:
        ...
        reranked = [
            ScoredChunk(
                chunk=item.chunk, score=float(score), retrieval_stage="cross_encoder"
            )
        ...
        context_block = "\n".join(
            [f"[{res.chunk.chunk_id}] {res.chunk.text}" for res in top_results]
        )
...
if __name__ == "__main__":
    ...
    document_chunks = chunker.split_text(
        transit_corpus,
        doc_id_prefix="infrastructure_2026",
        metadata={"source": "regional_plan"},
    )
